...
namespace Jint.Native {
    ...
    public class PropertyDescriptor<T> : PropertyDescriptor
        where T : JsInstance {
        public PropertyDescriptor(IGlobal global, JsDictionaryObject owner, string name, JintFunc<T, JsInstance> get)
            : base(global, owner, name) {
            GetFunction = global.FunctionClass.New<T>(get);
        }

        public PropertyDescriptor(IGlobal global, JsDictionaryObject owner, string name, JintFunc<T, JsInstance> get, JintFunc<T, JsInstance[], JsInstance> set)
