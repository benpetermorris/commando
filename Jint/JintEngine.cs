...
{
    public delegate void JintAction();
    public delegate void JintAction<T1, T2>(T1 t1, T2 t2);
    public delegate void JintAction<T1, T2, T3>(T1 t1, T2 t2, T3 t3);
...
namespace Jint
{
    ...
    public class JintEngine
    {
        ...
        static string StringFormatHelper(string format, JsArray args)
        {
            int len = (int)args["length"].ToNumber();
            if (len < 0)
                len = 0;

            var res = new object[len];
            for (int i = 0; i < len; i++)
                res[i] = args[new JsNumber(i, JsUndefined.Instance)].Value;
